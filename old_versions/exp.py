...
def apply_theme_css():
    ...
    if st.session_state.theme == 'dark':
        css = """
        <style>
            /* DARK MODE: Original dark background, forcing ALL text to white */
            ...
            /* Explicitly ensure all text elements are white with !important */
            .stMarkdown, label, h1, h2, h3, h4, 
            .st-bh, .st-bb, /* General Streamlit text components */
            [data-testid="stTitle"], [data-testid="stHeader"], [data-testid="stSidebarHeader"],
            .stSelectbox, .stSlider, .stRadio > label, /* Form components */
        ...
        # LIGHT MODE: Warmer, off-white background with dark text
        css = """
        <style>
            /* Main Content and Background - Warmer light tone */
            [data-testid="stAppViewContainer"] {
                background-color: #FAF8F1; /* Soft Off-White/Cream */
            }
            ...
            /* Explicitly set all standard text and form elements to dark color for max readability */
            .stMarkdown, label, [data-testid="stMetric"], 
            .stSelectbox, .stSlider, .stRadio > label,
            
            /* Target Streamlit Headings with data-testid and h tags + !important */
            h1, h2, h3, h4, 
            [data-testid="stTitle"], 
...
with col_main:
    ...
    # NEW: Persistent container for transient feedback messages
...
if run:
    ...
    while run and cap.isOpened():
        ...
        # --- NEW: Cleanup logic inside the loop to clear old feedback ---
        # Clear the feedback container if the display time has expired
                ...
                if app_mode == "Data Collection Mode (ML Prep)":
                    ...
                    if gesture_text == "👍 THUMBS UP (Mode Toggle)":
                        ...
                        if current_time - st.session_state.last_toggle_time > 1.5:
                            ...
                            # FIXED: Use the persistent container to display the message
                            FEEDBACK_CONTAINER.info(f"Switched to {st.session_state.theme.capitalize()} Mode", icon="✨")
                            # Set a time when the feedback should disappear (3 seconds from now)
                            st.session_state.feedback_display_time = current_time + 3.0
